/* BIM Portfolio — Project Focus Carousel v1.5.0 / Reel Aura presentation */
(function () {
  'use strict';
  ...
  (function loadProfessionalBIMTheme(){
    ...
    loadCss('reel-aura.css?v='+version,'data-reel-aura-theme');
  }());
  ...
  /* Reel-inspired stacked project presentation.
     Desktop/tablet uses vertical sticky cards; mobile keeps a lighter stacked version. */
  function initReelAura(){
    var grid=document.querySelector('.project-grid');
    if(!grid)return false;
    var cards=Array.prototype.slice.call(grid.querySelectorAll('.project-card'));
    if(cards.length<2)return false;
    grid.classList.add('reel-aura-projects');
    cards.forEach(function(card,index){card.setAttribute('data-aura-index',String(index+1).padStart(2,'0'))});

    var reduced=window.matchMedia&&window.matchMedia('(prefers-reduced-motion: reduce)').matches;
    var ticking=false;
    function update(){
      ticking=false;
      if(reduced){cards.forEach(function(card){card.style.setProperty('--aura-scale','1');card.style.setProperty('--aura-lift','0px');card.style.setProperty('--aura-rotate','0deg');card.style.opacity='1';card.style.filter='none'});return}
      var top=parseFloat(getComputedStyle(document.documentElement).getPropertyValue('--aura-top'))||92;
      cards.forEach(function(card,index){
        ...
        if(next){
          ...
          var range=Math.max(180,rect.height*.42);
          progress=Math.max(0,Math.min(1,(top+range-nextTop)/range));
        }
        var scale=1-progress*.055;
        var lift=-progress*10;
        var rotate=(index%2===0?-1:1)*progress*.35;
        card.style.setProperty('--aura-scale',scale.toFixed(4));
        card.style.setProperty('--aura-lift',lift.toFixed(1)+'px');
        card.style.setProperty('--aura-rotate',rotate.toFixed(3)+'deg');
        card.style.opacity=String(1-progress*.1);
        card.style.filter='brightness('+(1-progress*.07).toFixed(3)+')';
      });
    }
    function request(){if(ticking)return;ticking=true;window.requestAnimationFrame(update)}
  ...
  var auraEnabled=initReelAura();

  /* Keep the original horizontal focus carousel available as a fallback only.
     The new reel presentation owns .project-grid when enabled. */
  function initHorizontalFallback(){
    ...
    var style=document.createElement('style');style.setAttribute('data-project-focus-carousel','1.5.0');style.textContent='';document.head.appendChild(style);
    ...
    function start(){stop();if(reduced||document.hidden||dragging||hovered)return;timer=setInterval(function(){if(dragging||hovered||document.hidden)return;var next=(active+1)%cards.length;applyState(next);center(next,true)},2000)}
  ...
  if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',initHorizontalFallback,{once:true});else initHorizontalFallback();
}());
