...
(function () {
  ...
  function init() {
    ...
    /* script.js has the legacy 1s timer. Neutralize only its programmatic scroll methods. */
    ...
    function layout() {
      ...
      var cardWidth;
      if (window.innerWidth <= 700) cardWidth = Math.max(220, width - 56);
      else if (window.innerWidth <= 1050) cardWidth = Math.max(260, (width - gap) / 2);
      else cardWidth = Math.max(280, (width - gap * 2) / 3);
    ...
    function center(index, smooth) {
      nativeScrollTo(targetFor(index), (!reduced && smooth) ? 'smooth' : 'auto');
    }
